...
@pytest.mark.parametrize("actor,target,reach,walk", [
    ((26.0195, 39.8555), (24.5, 41.5), 10, False),
    ((3, 4), (0, 0), 5, False),  # Exact same inclusive boundary as fair.place.
    ((-12, 0), (0, 0), 10, True),
    ((0, 12), (0, 0), 10, True),
])
def test_native_build_reach_and_actor_side_approach(monkeypatch, actor, target, reach, walk):
    ...
    fair.approach_build(SimpleNamespace(x=target[0], y=target[1]))
...
def test_build_approach_failure_is_not_preflight_rejection(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match="native pathfinder"):
        fair.approach_build(SimpleNamespace(x=1, y=2))
